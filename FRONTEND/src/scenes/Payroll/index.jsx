...
const Payroll = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(
          "http://localhost:5001/api/get_employees"
        );
  ...
  const fetchAllowances = async () => {
    try {
      const response = await axiosInstance.get(
        "http://localhost:5001/api/get_allowances"
      );
  ...
  const handleAddAllowance = async () => {
    try {
      await axiosInstance.post(
        "http://localhost:5001/api/add_allowance",
        newAllowanceData
      );
      fetchAllowances();
      setOpenDialog(false);
  ...
  const handleDialogSave = async () => {
    try {
      await axiosInstance.put(
        `http://localhost:5001/api/edit_allowance/${editedAllowance._id}`,
        editedAllowance
      );
      fetchAllowances();
  ...
  const handleDeleteAllowance = async (id) => {
    try {
      await axiosInstance.delete(
        `http://localhost:5001/api/delete_allowance/${id}`
      );
      fetchAllowances();
    } catch (error) {
  ...
  const fetchBasicSalaries = async () => {
    try {
      const response = await axiosInstance.get(
        "http://localhost:5001/api/get_basic_salaries"
      );
  ...
  const handleAddBasicSalary = async () => {
    try {
      await axiosInstance.post(
        "http://localhost:5001/api/add_basic_salary",
  ...
  const handleSaveEditedDesignationSalary = async () => {
    try {
      await axiosInstance.put(
        `http://localhost:5001/api/edit_basic_salary/${editedDesignationSalary._id}`,
  ...
  const handleDeleteDesignationSalary = async (id) => {
    try {
      await axiosInstance.delete(
        `http://localhost:5001/api/delete_basic_salary/${id}`
      );
      fetchBasicSalaries();
    } catch (error) {
  ...
  const fetchDeductions = async () => {
    try {
      const response = await axiosInstance.get(
        "http://localhost:5001/api/get_deductions"
      );
  ...
  const handleAddDeduction = async () => {
    try {
      await axiosInstance.post(
        "http://localhost:5001/api/add_deduction",
        newDeductionData
      );
      fetchDeductions();
      setIsDeductionDialogOpen(false);
  ...
  const handleSaveEditedDeduction = async () => {
    try {
      await axiosInstance.put(
        `http://localhost:5001/api/edit_deduction/${editedDeduction._id}`,
        editedDeduction
      );
      fetchDeductions();
      setIsEditDeductionDialogOpen(false);
  ...
  const handleDeleteDeduction = async (id) => {
    try {
      await axiosInstance.delete(
        `http://localhost:5001/api/delete_deduction/${id}`
      );
      fetchDeductions();
    } catch (error) {
  ...
  const fetchPayslipData = async (id) => {
    try {
      const response = await axiosInstance.get(
        `http://localhost:5001/api/payslip/${id}`
      );
  ...
  return (
    <Box m="20px">
      ...
      <Dialog
        ...
        onExited={() => setEditedAllowance(null)}
      >
      ...
      {activeTab === "Deductions" && (
        <Box>
          ...
          <Dialog
            open={isEditDeductionDialogOpen}
            onClose={handleEditDeductionDialogClose}
          >
              ...
              <table style={{ width: "100%" }}>
                <tbody>
                  ...
                  <tr>
                    <td>Basic Salary:</td>
                    <td>
                      PKR {payslipData && payslipData.basicSalary.toFixed(2)}
                    </td>
                  </tr>
                  <tr>
                      ...
                      <ul>
                        {payslipData &&
                          payslipData.allowances.map((allowance, index) => (
                            <li key={index}>
                              {allowance.name}: PKR {allowance.value.toFixed(2)}
                            </li>
                          ))}
                      </ul>
                    </td>
                  ...
                  <tr>
                    <td>Salary:</td>
                    <td>
                      PKR {payslipData && payslipData.sub_total.toFixed(2)}
                    </td>
                  </tr>
                  <tr>
                    <td colSpan="2">
                  ...
                  <tr>
                    <td>Tax Deducted:</td>
                    <td>
                      PKR {payslipData && payslipData.taxAmount.toFixed(2)}
                    </td>
                  </tr>
                  <tr>
                    <td colSpan="2">
                  ...
                  <tr>
                    <td>Total Salary:</td>
                    <td>
                      PKR {payslipData && payslipData.totalSalary.toFixed(2)}
                    </td>
                  </tr>
                </tbody>
              </table>
