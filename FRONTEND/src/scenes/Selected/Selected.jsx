...
const Selected = () => {
  ...
  useEffect(() => {
    ...
    const filteredApplicants = applicantData.filter(
      (applicant) => applicant.selected && applicant.shortlisted
    );
    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(
          `http://localhost:5001/api/applied-applicants/${selectedJob}`
        );
  ...
  const fetchJobDetails = async (jobId) => {
    try {
      const response = await axiosInstance.get(
        `http://localhost:5001/api/get_job/${jobId}`
      );
    ...
    try {
      if (selectedAction === "select" || selectedAction === "unselect") {
        await axiosInstance.post(
          `http://localhost:5001/api/${selectedAction}/${selectedApplicantId}`,
          { jobId: selectedJob }
        );
        const response = await axiosInstance.get(
          `http://localhost:5001/api/applied-applicants/${selectedJob}`
      ...
      console.error(
        `Error ${
          selectedAction === "select" ? "selecting" : "unselecting"
        } applicant:`,
        error
      );
    }
      ...
      <Box mt={3} style={{ height: 400, width: "100%" }}>
        <DataGrid
          ...
          columns={[
            ...
            {
              field: "status one",
              headerName: "Status Shortlisted",
              width: 150,
              renderCell: (params) => (
                <Typography
                  style={{
                    color: params.row.shortlisted ? blue[500] : grey[500],
                  }}
                >
            ...
            },
            {
              ...
              renderCell: (params) => (
                <Typography
                  style={{ color: params.row.selected ? blue[500] : grey[500] }}
                >
