...
const LeaveTypes = () => {
  ...
  const handleEditLeave = async (id) => {
    try {
      ...
      const response = await axiosInstance.get(
        `http://localhost:5001/api/get_leave/${id}`
      );
    ...
    try {
      const response = editedLeave._id
        ? await axiosInstance.put(
            `http://localhost:5001/api/edit_leave/${editedLeave._id}`,
            editedLeave
          )
        : await axiosInstance.post(
            "http://localhost:5001/api/add_leave",
            editedLeave
          );
  ...
  const fetchLeaveData = () => {
    axiosInstance
      .get("http://localhost:5001/api/get_leaves")
      .then((response) => {
  ...
  const handleActivateDeactivate = async (id, status) => {
    try {
      ...
      const response = await axiosInstance.put(
        `http://localhost:5001/api/activate_deactivate_leave/${id}`,
        { status: newStatus }
      );
  ...
  const handleConfirmDelete = () => {
    ...
    axiosInstance
      .delete(`http://localhost:5001/api/delete_leave/${id}`)
      .then((response) => {
