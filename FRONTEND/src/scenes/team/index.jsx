...
const Team = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await axiosInstance.get(
        `http://localhost:5001/api/search_employee/${searchQuery}`
      );
  ...
  const handleConfirmDelete = () => {
    ...
    axiosInstance
      .delete(`http://localhost:5001/api/delete_employee/${id}`)
      .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("http://localhost:5001/api/add_employee", {
        ...values,
        profilepic: pic,
        profilecv: cv,
      });
