    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(
          `http://localhost:5001/api/applicants/${currentUser._id}`
        );
    ...
    try {
      switch (field) {
        case "username":
          ...
          await axiosInstance.put(`/applicantsName/${currentUser._id}`, {
            name: username,
          });
          break;
        case "email":
          ...
          // Save email changes
          setEditEmail(false);
          await axiosInstance.put(`/applicantsEmail/${currentUser._id}`, {
            email,
          });
          break;
        case "password":
          ...
          await axiosInstance.put(`/applicantsPassword/${currentUser._id}`, {
            password,
          });
          break;
        default:
    ...
    <Box mt={5} justifyContent="left">
      <Box mt={5}>
        ...
        >
          <Typography variant="body2" color="textSecondary">
            <b>NOTE:</b> Whenever you update your username, email or password,
            make sure to <b>logout</b> so that the changes are visible
            system-wide. Thank you!
          </Typography>
        </Paper>
      </Box>
      <Box
      ...
      >
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={2} style={{ display: "flex", alignItems: "center" }}>
            {editEmail ? (
              <IconButton onClick={() => handleSave("email")} color="primary">
                <CheckCircleIcon />
              </IconButton>
            ) : (
              <IconButton
                ...
                {profilePicExists ? (
                  ...
                  >
                    ...
                    <Typography variant="body2" style={{ marginTop: "50px" }}>
                      {displayFileName(selectedImageFileName)}
                    </Typography>
                  </div>
                ) : (
                  <Typography variant="body2" style={{ marginTop: "50px" }}>
                ...
                {profileCVExists ? (
                  ...
                  >
                    ...
                    <Typography variant="body2" style={{ marginTop: "30px" }}>
                      {displayFileName(selectedCVFileName)}
                    </Typography>
                  </div>
                ) : (
                  <Typography variant="body2" style={{ marginTop: "30px" }}>
